<div>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-extrabold text-gray-900 dark:text-white">Videos</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage video content</p>
            </div>
            <a href="{{ route('admin.videos.create') }}" class="flex items-center gap-2 bg-vnn-red text-white text-sm font-bold px-4 py-2.5 rounded-lg hover:bg-vnn-red-dark transition shadow-lg shadow-vnn-red/20">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            ...
                            <th wire:click="sortBy('status')" class="text-left px-5 py-3.5 font-semibold text-gray-600 dark:text-gray-400 cursor-pointer hover:text-vnn-red hidden md:table-cell">Status</th>
                            <th wire:click="sortBy('views')" class="text-left px-5 py-3.5 font-semibold text-gray-600 dark:text-gray-400 cursor-pointer hover:text-vnn-red hidden md:table-cell">Views</th>
                            <th wire:click="sortBy('is_top')" class="text-left px-5 py-3.5 font-semibold text-gray-600 dark:text-gray-400 cursor-pointer hover:text-vnn-red hidden md:table-cell">Top</th>
                            <th class="text-left px-5 py-3.5 font-semibold text-gray-600 dark:text-gray-400 hidden md:table-cell">Duration</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30">
                            ...
                            <td class="px-5 py-3.5 text-gray-500 dark:text-gray-400 hidden md:table-cell text-xs">{{ $video->category?->name ?? '—' }}</td>
                            ...
                            <td class="px-5 py-3.5 text-gray-500 dark:text-gray-400 hidden md:table-cell">{{ number_format($video->views) }}</td>
                            <td class="px-5 py-3.5 hidden md:table-cell">
                                <button wire:click="toggleTop({{ $video->id }})" class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold transition {{ $video->is_top ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400' }}">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                    {{ $video->is_top ? 'Top' : 'Set Top' }}
                                </button>
                            </td>
                            <td class="px-5 py-3.5 text-gray-500 dark:text-gray-400 hidden md:table-cell text-xs">{{ $video->duration ? gmdate('i:s', $video->duration) : '—' }}</td>
                            <td class="px-5 py-3.5">
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center text-gray-400">No videos found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
