            ...
            <div class="mb-6 text-center">
                <a href="/" wire:navigate class="flex flex-col items-center">
                    <div class="flex items-center gap-3">
                        <div class="w-14 h-14 bg-white rounded-xl flex items-center justify-center shadow-lg overflow-hidden">
                            @php $siteLogo = \App\Models\Setting::where('key', 'site_logo')->value('value'); @endphp
                            @if($siteLogo)
                            <img src="{{ asset('storage/' . $siteLogo) }}" alt="{{ config('app.name') }}" class="w-full h-full object-contain p-1" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                            <div class="hidden w-full h-full items-center justify-center">
                                <span class="text-vnn-blue font-extrabold text-2xl">V</span>
                            </div>
                            @else
                            <span class="text-vnn-blue font-extrabold text-2xl">V</span>
                            @endif
                    ...
                    <p class="text-gray-300 text-xs mt-2 italic">Truth. Speed. Impact.</p>
                </a>
            </div>
