...
<body class="font-heading antialiased bg-vnn-gray dark:bg-vnn-dark overflow-x-hidden" :class="{ 'overflow-hidden': mobileOpen }">
    ...
    <header class="bg-white dark:bg-vnn-dark-light border-b border-gray-200 dark:border-vnn-dark-light">
        <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <div class="hidden lg:flex items-center gap-3">
                ...
                @auth
                    <a href="{{ route('admin.dashboard') }}" class="bg-vnn-red text-white text-sm font-semibold px-5 py-2 rounded hover:bg-vnn-red-dark transition">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="bg-vnn-red text-white text-sm font-semibold px-5 py-2 rounded hover:bg-vnn-red-dark transition">Sign In</a>
                @endauth
            </div>
        </div>
    ...
    <div x-show="mobileOpen" x-cloak @click.away="mobileOpen = false" class="fixed inset-0 z-50 lg:hidden">
        ...
        <div class="fixed top-0 left-0 bottom-0 w-72 max-w-[85vw] bg-white dark:bg-vnn-dark shadow-2xl overflow-y-auto">
            ...
            <div class="px-4 pb-6">
                @auth
                    <a href="{{ route('admin.dashboard') }}" class="block w-full text-center bg-vnn-red text-white text-sm font-bold py-3 rounded hover:bg-vnn-red-dark transition">Dashboard</a>
                @else
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div>
                    ...
                    <div class="flex gap-3 mb-6">
                        <a href="#" @click.prevent="showSocials = true" class="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-vnn-red transition"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"/></svg></a>
                        <a href="#" @click.prevent="showSocials = true" class="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-vnn-red transition"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg></a>
                        <a href="#" @click.prevent="showSocials = true" class="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-vnn-red transition"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/></svg></a>
                        <a href="#" @click.prevent="showSocials = true" class="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-vnn-red transition"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.477 2 2 6.477 2 12v0c0 5.523 4.477 10 10 10s10-4.477 10-10-4.477-10-10-10zm3.5 14.5H14v-2.5c0-.828-.672-1.5-1.5-1.5s-1.5.672-1.5 1.5v2.5H8.5v-8h2.5v1.5c.5-.5 1.2-1 2.5-1 1.933 0 3.5 1.567 3.5 3.5v4zM7.5 8.5c-.828 0-1.5-.672-1.5-1.5s.672-1.5 1.5-1.5S9 6.172 9 7s-.672 1.5-1.5 1.5z"/></svg></a>
                        <a href="#" @click.prevent="showSocials = true" class="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-vnn-red transition"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.248c-.237.3-.527.558-.855.753-.03.296-.044.599-.082.897-.46 3.624-1.94 6.143-5.26 7.957-2.246 1.229-4.613 1.38-7.086.384.987.079 1.945-.174 2.79-.672 1.434-.827 2.447-1.93 3.005-3.493.311-.882.088-1.653-.608-2.22-.573-.468-1.185-.684-1.897-.506-.808.202-1.303.716-1.502 1.513-.201.809-.047 1.557.425 2.236.12.173.132.2-.057.072-1.937-2.066-1.87-4.784.201-6.904 1.04-1.064 2.337-1.639 3.914-1.715.899-.043 1.686.202 2.271.899.491.586.539 1.29.376 2.005-.159.7-.585 1.221-1.235 1.55-.342.173-.397.04-.087-.141.385-.225.596-.569.643-1.008.013-.255-.026-.332-.285-.383-.586-.117-1.151-.097-1.54.364-.382.453-.52.988-.492 1.567.042.89.437 1.505 1.213 1.867.429.2.891.305 1.36.338.886.062 1.639-.24 2.143-.995.317-.475.45-1.016.381-1.59-.049-.407-.196-.778-.435-1.092-1.17-1.53-3.478-1.71-4.964-.427-1.45 1.252-1.772 3.188-.61 4.893.425.623.981 1.09 1.672 1.383 1.09.462 2.221.25 3.112-.597.53-.503.877-1.118 1.048-1.844.144-.615.017-1.211-.35-1.736-.28-.4-.344-.439-.054-.195.526.443.79 1.012.747 1.703-.061.988-.465 1.837-1.153 2.531-1.04 1.05-2.341 1.534-3.842 1.471-1.235-.052-2.335-.44-3.237-1.256-1.23-1.114-1.843-2.506-1.808-4.148.036-1.664.643-3.093 1.831-4.258 1.306-1.28 2.903-1.848 4.714-1.72 1.469.104 2.734.672 3.763 1.72.157.16.308.325.382.533.059.165.017.227-.17.113z"/></svg></a>
                    </div>
                    <div id="subscribe">
    ...
    @php
        $socials = [
            'facebook' => ['url' => \App\Models\Setting::where('key', 'social.facebook_url')->value('value') ?? '#', 'color' => 'bg-[#1877F2]', 'hover' => 'hover:bg-[#166FE5]', 'label' => 'Facebook', 'icon' => '<path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"/>'],
            'twitter' => ['url' => \App\Models\Setting::where('key', 'social.twitter_url')->value('value') ?? '#', 'color' => 'bg-black', 'hover' => 'hover:bg-gray-800', 'label' => 'X / Twitter', 'icon' => '<path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>'],
            'instagram' => ['url' => \App\Models\Setting::where('key', 'social.instagram_url')->value('value') ?? '#', 'color' => 'bg-gradient-to-br from-[#F58529] via-[#DD2A7B] to-[#8134AF]', 'hover' => 'hover:opacity-90', 'label' => 'Instagram', 'icon' => '<path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>'],
            'linkedin' => ['url' => \App\Models\Setting::where('key', 'social.linkedin_url')->value('value') ?? '#', 'color' => 'bg-[#0A66C2]', 'hover' => 'hover:bg-[#095196]', 'label' => 'LinkedIn', 'icon' => '<path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>'],
            'youtube' => ['url' => \App\Models\Setting::where('key', 'social.youtube_url')->value('value') ?? '#', 'color' => 'bg-[#FF0000]', 'hover' => 'hover:bg-[#CC0000]', 'label' => 'YouTube', 'icon' => '<path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>'],
        ];
    @endphp
    <div x-show="showSocials" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center p-4" @keydown.escape.window="showSocials = false">
        <div class="fixed inset-0 bg-black/70" @click="showSocials = false"></div>
        <div class="relative bg-white dark:bg-vnn-dark-light rounded-2xl shadow-2xl max-w-md w-full p-8 text-center" @click.away="showSocials = false">
            <button @click="showSocials = false" class="absolute top-3 right-3 w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-white rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
            <div class="w-20 h-20 bg-gradient-to-br from-vnn-red to-vnn-red-dark rounded-full flex items-center justify-center mx-auto mb-5 shadow-lg">
                <svg class="w-10 h-10 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>
            </div>
            <h3 class="text-2xl font-extrabold text-vnn-dark dark:text-white font-heading">Follow Us</h3>
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-3 font-body leading-relaxed">Stay connected with Verve News Network on your favorite social platforms.</p>
            <div class="mt-6 space-y-3">
                @foreach($socials as $key => $social)
                    <a href="{{ $social['url'] }}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 w-full px-4 py-3 {{ $social['color'] }} {{ $social['hover'] }} text-white rounded-xl font-semibold text-sm transition active:scale-[0.98] shadow-md hover:shadow-lg">
                        <svg class="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 24 24">{!! $social['icon'] !!}</svg>
                        <span>{{ $social['label'] }}</span>
                        <svg class="w-4 h-4 ml-auto opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                    </a>
                @endforeach
            </div>
            <button @click="showSocials = false" class="mt-6 w-full bg-vnn-red text-white font-bold py-3 px-6 rounded-xl hover:bg-vnn-red-dark transition active:scale-[0.98]">Close</button>
        </div>
    </div>
