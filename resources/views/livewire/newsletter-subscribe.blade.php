<div>
    ...
    @else
        <form wire:submit="subscribe" class="flex gap-2">
            <input wire:model="email" type="email" placeholder="Your email" class="flex-1 px-3 py-2.5 text-sm text-gray-900 rounded border-0 focus:outline-none focus:ring-1 focus:ring-vnn-red font-body" required>
            <button type="submit" class="bg-vnn-red text-white px-4 py-2.5 text-xs font-bold rounded hover:bg-vnn-red-dark transition whitespace-nowrap">Subscribe</button>
            @error('email') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
        </form>
    @endif
</div>
